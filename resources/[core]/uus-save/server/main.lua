...
MySQL.Async.fetchAll("SELECT * FROM `darah_armour`",{},
function(data)
...
AddEventHandler("notbad-save-health-armour:loadData", function()
    ...
    if Statuses[identifier] == nil then 
        MySQL.Async.execute('INSERT INTO `darah_armour` (`identifier`, `status`) VALUES (@identifier, @status)', {
    ...
    local status = MySQL.Sync.fetchAll("SELECT `status` FROM `darah_armour` WHERE `identifier` = '" .. identifier .. "' LIMIT 1")
    if status[1] then
...
function SaveHealthAndArmour(src)
    ...
    MySQL.Async.execute("UPDATE `darah_armour` SET `status` = '" .. jsonData .. "' WHERE `identifier` = '" .. identifier .. "'")
end