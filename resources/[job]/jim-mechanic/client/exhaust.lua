...
RegisterNetEvent('jim-mechanic:client:Exhaust:Check', function()
	...
	if DoesEntityExist(vehicle) then
		...
		if GetVehicleMod(vehicle, 4) == -1 then	stockinstall = Loc[Config.Lan]["common"].current icon = "fas fa-check" disabled = true else stockinstall = "" end
		local ExhaustMenu = {
			{ isMenuHeader = true, icon = "exhaust", header = searchCar(vehicle)..Loc[Config.Lan]["exhaust"].menuheader, txt = Loc[Config.Lan]["common"].amountoption..#validMods+1, },
			{ icon = "fas fa-circle-xmark", header = "", txt = string.gsub(Loc[Config.Lan]["common"].close, "❌ ", ""), params = { event = "jim-mechanic:client:Menu:Close" } },
			{ icon = icon, isMenuHeader = disabled, header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall,	params = { event = "jim-mechanic:client:Exhaust:Apply", args = { id = -1 } } }, }
