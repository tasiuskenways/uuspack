...
RegisterNetEvent('jim-mechanic:client:Interior:Apply', function(data)
	...
	elseif GetVehicleMod(vehicle, tonumber(data.bumperid)) ~= tonumber(data.mod) then
		...
		{ animDict = "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", anim = "machinic_loop_mechandplayer", flags = 16, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
...
RegisterNetEvent('jim-mechanic:client:Interior:Choose', function(mod)
	...
	if not outCar() then return end
	local vehicle = nil
	if IsPedInAnyVehicle(PlayerPedId(), true) then vehicle = GetVehiclePedIsIn(PlayerPedId(), true)
	...
	if DoesEntityExist(vehicle) then
		...
		local ModMenu = {
				{ isMenuHeader = true, icon = "internals",header = searchCar(vehicle)..Loc[Config.Lan]["interior"].menuheader, txt = Loc[Config.Lan]["common"].amountoption..#validMods+1, },
				{ icon = "fas fa-circle-arrow-left", header = "", txt = string.gsub(Loc[Config.Lan]["common"].ret, "⬅️ ", ""), params = { event = "jim-mechanic:client:Interior:Check" } },
				{ icon = icon, isMenuHeader = disabled, header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall, params = { event = "jim-mechanic:client:Interior:Apply", args = { mod = -1, bumperid = tonumber(mod) } } }}
