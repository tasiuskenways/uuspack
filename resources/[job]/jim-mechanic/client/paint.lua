...
RegisterNetEvent('jim-mechanic:client:Paints:Apply', function(data)
	...
	local vehicle
	if not IsPedInAnyVehicle(playerPed, false) then	vehicle = getClosest(coords) pushVehicle(vehicle) lookVeh(vehicle) else
	...
	elseif colourCheck ~= data.id then
		...
		CreateThread(function()
			while spraying do
				...
				local spray = StartNetworkedParticleFxNonLoopedAtCoord("scr_wheel_burnout", canCoords.x, canCoords.y, canCoords.z + 0.4, 0.0, 0.0, GetEntityHeading(playerPed), 0.2, 0.0, 0.0, 0.0)
				Wait(150)
			end
		end)
		...
		{ task = settask, animDict = setanimDict, anim = setanim, flags = setflag, }, {}, {}, function()  SetVehicleModKit(vehicle, 0)
