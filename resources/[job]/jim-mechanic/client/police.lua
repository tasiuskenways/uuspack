...
Config.PoliceLocations = {
	...
	{ coords = vector3(445.63, -987.36, 25.14),  heading = 0, },  -- MRPD UNDERGROUND PARKING
	{ coords = vector3(342.51, -570.98, 28.8),   heading = 250.0, }, -- PILL BOX GARAGE
	{ coords = vector3(-479.71, 6016.54, 31.34), heading = 135.0, }, -- Paleto Sheriff Office
}

local bench = {}
CreateThread(function()
	for k, v in pairs(Config.PoliceLocations) do
		RequestModel(`gr_prop_gr_bench_03a`)
		while not HasModelLoaded(`gr_prop_gr_bench_03a`) do Citizen.Wait(2) end
		bench[#bench + 1] = CreateObject(`gr_prop_gr_bench_03a`, v.coords.x, v.coords.y, v.coords.z - 2.4, false, false,
			false)
		SetEntityHeading(bench[#bench], v.heading)
		FreezeEntityPosition(bench[#bench], true)
		exports['qb-target']:AddBoxZone("bench" .. k, v.coords, 1.2, 4.2,
			{
				name = "bench" .. k,
				heading = v.heading,
...
RegisterNetEvent('jim-mechanic:client:Police:Menu', function()
	...
	if IsPedInAnyVehicle(PlayerPedId(), false) then
		...
		PoliceMenu[#PoliceMenu + 1] = {
			...
			txt = "",
			params = { event = "jim-mechanic:client:Police:Repair" },
		}
		PoliceMenu[#PoliceMenu + 1] = {
			icon = "fas fa-toolbox",
			header = Loc[Config.Lan]["police"].extras,
			txt = "",
			params = { event = "jim-mechanic:client:Police:Extra" },
		}
		PoliceMenu[#PoliceMenu + 1] = {
			icon = "fas fa-address-card",
			header = Loc[Config.Lan]["police"].plates,
			txt = "",
			params = { event = "jim-mechanic:client:Police:Plates" },
		}
		if GetNumVehicleMods(vehicle, 48) > 0 or GetVehicleLiveryCount(vehicle) > -1 then
			PoliceMenu[#PoliceMenu + 1] = {
				icon = "fas fa-paint-roller",
				header = Loc[Config.Lan]["police"].livery,
				txt = "",
				params = { event = "jim-mechanic:client:Police:Livery" },
			}
		end
		if GetNumVehicleMods(vehicle, 0) ~= 0 then
			PoliceMenu[#PoliceMenu + 1] = {
				icon = "fas fa-car-burst",
				header = Loc[Config.Lan]["police"].spoiler,
				txt = "",
				params = { event = "jim-mechanic:client:Police:Spoiler" },
			}
		end
		PoliceMenu[#PoliceMenu + 1] = {
			icon = "fas fa-brush",
			...
			txt = "",
			params = { event = "jim-mechanic:client:Police:Paint" },
		}
	...
	for i = 0, 20 do
		if DoesExtraExist(vehicle, i) then
			...
			ExtraMenu[#ExtraMenu + 1] = {
				...
				header = "Extra " .. i,
				txt = "",
				params = { event = "jim-mechanic:client:Police:Extra:Apply", args = { id = i }, },
			}
		end
	end
	if hadMod then
		...
		if GetNumVehicleMods(vehicle, 48) == 0 and GetVehicleLiveryCount(vehicle) ~= 0 then
			...
			for i = 0, GetVehicleLiveryCount(vehicle) - 1 do
				...
				if i ~= 0 then
					validMods[i] = {
						id = i,
						name = Loc[Config.Lan]["police"].livery .. " " .. i,
						install = txt
					}
				end
			end
	...
	if IsPedInAnyVehicle(playerPed, false) then
		...
		if DoesEntityExist(vehicle) then
			local PlateMenu = {
				{
					icon = "fas fa-address-card",
					header = searchCar(vehicle) .. Loc[Config.Lan]["plates"]
						.menuheader2,
					isMenuHeader = true
				},
				{
					...
					header = "",
					txt =
						string.gsub(Loc[Config.Lan]["common"].ret, "⬅️ ", ""),
					params = {
						event = "jim-mechanic:client:Police:Menu" }
				} }
...
RegisterNetEvent('jim-mechanic:client:Police:Plates:Apply', function(index)
	...
	if GetVehicleNumberPlateTextIndex(vehicle) == tonumber(index) then
		TriggerEvent("QBCore:Notify", Loc[Config.Lan]["plates"].already, "error")
		...
		local spoilerMenu = {
			{
				icon = "fas fa-car-burst",
				isMenuHeader = true,
				header = Loc[Config.Lan]["spoilers"]
					.menuheader,
				txt =
					Loc[Config.Lan]["common"].amountoption .. #validMods + 1,
			},
			{
				icon = "fas fa-circle-arrow-left",
				header = "",
				txt = string.gsub(
					Loc[Config.Lan]["common"].ret, "⬅️ ", ""),
				params = {
					event = "jim-mechanic:client:Police:Menu" }
			},
			{
...
RegisterNetEvent('jim-mechanic:client:Police:Paints:Choose', function(data)
	...
	if DoesEntityExist(vehicle) then
		exports['qb-menu']:openMenu({
			{
				icon = "fas fa-brush",
				header = data .. Loc[Config.Lan]["paint"].menuheader,
				txt = "",
				isMenuHeader = true
			},
			{
			...
			},
			{
				...
				params = {
					...
					args = { paint = data, finish = Loc[Config.Lan]["paint"].classic }
				}
			},
			{
	...
	if data.finish == Loc[Config.Lan]["paint"].classic then
		for k, v in pairs(Loc[Config.Lan].vehicleResprayOptionsClassic) do
			...
			PaintMenu[#PaintMenu + 1] = {
				...
				txt = installed,
				params = {
					event = 'jim-mechanic:client:Police:Paints:Apply',
					args = { paint = data.paint, id = v.id, name = v.name, finish = data.finish }
				}
			}
		end
