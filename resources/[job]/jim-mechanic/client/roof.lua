...
RegisterNetEvent('jim-mechanic:client:Roof:Apply', function(mod)
	...
	elseif GetVehicleMod(vehicle, 10) ~= tonumber(mod) then
		...
		{ task = "WORLD_HUMAN_WELDING" }, {}, {}, function() SetVehicleModKit(vehicle, 0)
...
RegisterNetEvent('jim-mechanic:client:Roof:Check', function()
	...
	local RoofMenu = {
			{ isMenuHeader = true, icon = "roof", header = searchCar(vehicle)..Loc[Config.Lan]["roof"].menuheader, txt = Loc[Config.Lan]["common"].amountoption..GetNumVehicleMods(vehicle, 10)+1,	},
			{ icon = "fas fa-circle-xmark", header = "", txt = string.gsub(Loc[Config.Lan]["common"].close, "❌ ", ""), params = { event = "jim-mechanic:client:Menu:Close" } },
			{ icon = icon, isMenuHeader = disabled, header = Loc[Config.Lan]["common"].stock, txt = stockinstall,	params = { event = "jim-mechanic:client:Roof:Apply", args = -1 } } }
		for k,v in pairs(validMods) do
			local icon = "" local disabled = false
			if GetVehicleMod(vehicle, 10) == v.id then icon = "fas fa-check" disabled = true end
			RoofMenu[#RoofMenu + 1] = { icon = icon, isMenuHeader = disabled, header = k..". "..v.name, txt = v.install, params = { event = 'jim-mechanic:client:Roof:Apply', args = tostring(v.id) } }
		end
