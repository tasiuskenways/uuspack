			...
			if v.name == b.name and k ~= l then
				if not v.unique then
					if Config.Debug then
						...
						print("^5Debug^7: ^3StashTidy^7: ^2Removing Stash slot ^7[^6"..l.."^7] (^2x^6"..b.amount.."^7) ^2and Adding to Stash slot ^7[^6"..k.."^7] (^2x^6"..v.amount.."^7)")
					end
		...
		for k, v in pairs(craftable[ItemMake]) do
			for l, b in pairs(stashItems) do
				if k == b.name then
					...
					if Config.Debug then print("^5Debug^7: ^3Crafting^7: ^2Removing ^6"..QBCore.Shared.Items[k].label.." ^2x^6"..v.." ^2from stash^7: '^6"..Player.PlayerData.job.name.."Safe^7'") end
				end
			end
		end
	else
	...
	if Config.StashCraft then
		...
		for k, v in pairs(craftable[item]) do
			for _, b in pairs(stashItems) do
				if k == b.name and b.amount >= v then testtable[k] = true if Config.Debug then print("^5Debug^7: ^3Crafting^7: ^6"..b.label.." ^2x^6"..b.amount.." ^2found in stash^7: '^6"..Player.PlayerData.job.name.."Safe^7'") end end
			end
		end
	else
		for k, v in pairs(craftable[item]) do
			if QBCore.Functions.GetPlayer(source).Functions.GetItemByName(k) and QBCore.Functions.GetPlayer(source).Functions.GetItemByName(k).amount >= v then
				testtable[k] = true if Config.Debug then print("^5Debug^7: ^3Crafting^7: ^6"..k.." ^7(^2x^6"..v.."^7) ^2found^7") end
			end
		end
	end
	for k, v in pairs(testtable) do
		if not v then hasitem = false if Config.Debug then print("^5Debug^7: ^3Crafting^7: ^6"..QBCore.Shared.Items[k].label.." ^1NOT ^2found^7") end end
	end
	cb(hasitem)
end)