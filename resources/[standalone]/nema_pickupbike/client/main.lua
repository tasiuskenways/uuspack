...
exports['qb-target']:addModel(BikeModels, { {
...
AddEventHandler('nema_pickupbike:pickbike', function(target)
	...
	AttachEntityToEntity(target.entity, cache.ped, GetPedBoneIndex(cache.ped, Config.Bone),
		Config.Models[model].position.x, Config.Models[model].position.y, Config.Models[model].position.z,
		Config.Models[model].rotation.x, Config.Models[model].rotation.y, Config.Models[model].rotation.z, true, true,
		false,
		true, 1, true)
	while bike do
