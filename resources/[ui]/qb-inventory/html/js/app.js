...
function handleDragDrop() {
  ...
  $(".item-slot").droppable({
    ...
    drop: function (event, ui) {
      ...
      var toDataUnique = toInventory
        .find("[data-slot=" + toSlot + "]")
        .data("item");
      var fromDataUnique = fromInventory
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
  ...
  if (fromData !== undefined && fromData.amount >= $toAmount) {
    ...
    if (
      ...
      $.post(
        `https://${GetParentResourceName()}/PlayDropSound`,
        JSON.stringify({})
      );
      $.post(
        `https://${GetParentResourceName()}/SetInventoryData`,
...
(() => {
  ...
  Inventory.Error = function () {
    $.post(
      `https://${GetParentResourceName()}/PlayDropFail`,
      JSON.stringify({})
    );
  };

  Inventory.IsWeaponBlocked = function (WeaponName) {
    var DurabilityBlockedWeapons = ["weapon_unarmed", "weapon_stickybomb"];
  ...
  Inventory.Close = function () {
    ...
    $.post(
      `https://${GetParentResourceName()}/CloseInventory`,
      JSON.stringify({})
    );
